import type { Outcome, Question } from "~/models/questions";
...
export const useAppStore = defineStore("appStore", () => {
  ...
  const outcomes = ref<Array<Outcome>>([
    {
  ...
  const outcome = computed(() => {
    if (answers.value.length == 3) {
      const _outcome = outcomes.value.find((o) => {
        return (
          o.path[0] == answers.value[0] &&
          o.path[1] == answers.value[1] &&
          o.path[2] == answers.value[2]
        );
      });
      if (_outcome) {
        return _outcome;
